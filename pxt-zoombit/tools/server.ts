...
try {
  ...
  if (fs.existsSync(pxtEnv)) dotenv.config({ path: pxtEnv });
...
function normalizeHexOutputs(): { path: string, size: number } | null {
  try {
    const latest = findLatestHex();
    if (!latest) return null;
    // Delete all other .hex files first (except the latest)
    const entries = fs.readdirSync(builtDir).filter((n: string) => n.toLowerCase().endsWith('.hex'));
    ...
    if (path.resolve(latest.path) !== path.resolve(defaultHexPath)) {
      try { fs.renameSync(latest.path, defaultHexPath); } catch { /* fallback: copy */ try { fs.copyFileSync(latest.path, defaultHexPath); fs.unlinkSync(latest.path); } catch { } }
    }
...
function writeDebugLogSync(baseDir: string, filePrefix: string, content: string) {
  try {
  ...
  typeof payload.code === 'string' ||
      ...
      runBuild(async (err, stdout, stderr) => {
        if (err) {
          ...
          try { if (err && err.stack) logger.error('[BUILD][stack]\n' + String(err.stack)); } catch {}
          if (stderr) logger.error('[BUILD][stderr]\n' + stderr);
          if (stdout) logger.error('[BUILD][stdout]\n' + stdout);
            ...
            try {
              ...
              if (target && codeTxt) {
                ...
                if (fixed && fixed.trim()) {
                  ...
                  // Rebuild once
                  return runBuild((err2: any, stdout2: string, stderr2: string) => {
                    if (err2) {
                      ...
                      fs.stat(defaultHexPath, (statErr: any, stats: any) => {
                        if (statErr) {
                          ...
                          try { logger.info(`[BUILD] HEX ready (after auto-fix): ${latest.path} (${latest.size} bytes)`); } catch {}
                        ...
                        try { logger.info(`[BUILD] HEX ready (after auto-fix): ${defaultHexPath} (${stats.size} bytes)`); } catch {}
        ...
        fs.stat(defaultHexPath, (statErr: any, stats: any) => {
          if (statErr) {
            ...
            try { logger.info(`[BUILD] HEX ready: ${norm.path} (${norm.size} bytes)`); } catch {}
            const payloadOut = { success: true, message: 'Build succeeded', hex: norm.path, size: norm.size, durationMs: Date.now() - started } as any;
            sendJson(res, 200, payloadOut);
            sseBroadcast('build-succeeded', payloadOut);
            return;
          }
...
async function handleAiBuild(payload: any, res: any) {
  const started = Date.now();
  try {
  ...
  let aiCode = (typeof payload.code === 'string' && payload.code.trim()) ? String(payload.code) : tryRuleBasedGenerate(userPrompt);
    ...
    if (!aiCode && !hasOpenAIKey) {
      ...
      try { logger.warn('[AI-BUILD] ' + note); } catch {}
      sseBroadcast('ai-debug', { phase: 'ai-disabled', reason: 'no-api-key' });
      const plainStarted = Date.now();
    runBuild((err: any, stdout: string, stderr: string) => {
        if (err) {
      try { if (err && err.stack) logger.error('[BUILD][ai-disabled][stack]\n' + String(err.stack)); } catch {}
      const resp: any = { success: false, message: 'Build failed (AI disabled)', error: String(err), stack: (err && err.stack) ? String(err.stack) : undefined, stderr, stdout, durationMs: Date.now() - plainStarted };
        ...
        fs.stat(defaultHexPath, (statErr: any, stats: any) => {
          if (statErr) {
            const latest = findLatestHex();
            if (!latest) {
              const resp: any = { success: false, message: 'Build completed but HEX not found (AI disabled)', expected: defaultHexPath, builtDir, builtListing: listBuiltDir(), stderr, stdout, durationMs: Date.now() - plainStarted };
              sendJson(res, 500, resp);
              sseBroadcast('ai-build-failed', resp);
              return;
            }
            const norm = normalizeHexOutputs() || { path: latest.path, size: latest.size };
            try { logger.info(`[AI-BUILD] HEX ready (ai-disabled): ${norm.path} (${norm.size} bytes)`); } catch {}
            const resp: any = { success: true, message: note, hex: norm.path, size: norm.size, durationMs: Date.now() - plainStarted, aiDisabled: true };
          ...
          const norm = normalizeHexOutputs() || { path: defaultHexPath, size: stats.size };
          try { logger.info(`[AI-BUILD] HEX ready (ai-disabled): ${norm.path} (${norm.size} bytes)`); } catch {}
          const resp: any = { success: true, message: note, hex: norm.path, size: norm.size, mtime: stats.mtime, durationMs: Date.now() - plainStarted, aiDisabled: true };
          sendJson(res, 200, resp);
          sseBroadcast('ai-build-succeeded', resp);
        });
      });
    ...
    if (!aiCode) {
      ...
      try {
        ...
        if (debug) { try { logger.warn('[AI-BUILD] OpenAI request failed; falling back to plain build: ' + String(err)); } catch {} }
        ...
        const plainStarted = Date.now();
    runBuild((err2: any, stdout: string, stderr: string) => {
          if (err2) {
      try { if (err2 && err2.stack) logger.error('[BUILD][ai-fallback][stack]\n' + String(err2.stack)); } catch {}
      const resp: any = { success: false, message: 'Build failed (AI unavailable)', error: String(err2), stack: (err2 && err2.stack) ? String(err2.stack) : undefined, stderr, stdout, durationMs: Date.now() - plainStarted };
            sendJson(res, 500, resp);
            sseBroadcast('ai-build-failed', resp);
            return;
          }
          fs.stat(defaultHexPath, (statErr: any, stats: any) => {
            if (statErr) {
              const latest = findLatestHex();
              if (!latest) {
                const resp: any = { success: false, message: 'Build completed but HEX not found (AI unavailable)', expected: defaultHexPath, builtDir, builtListing: listBuiltDir(), stderr, stdout, durationMs: Date.now() - plainStarted };
                sendJson(res, 500, resp);
                sseBroadcast('ai-build-failed', resp);
                return;
              }
              try { logger.info(`[AI-BUILD] HEX ready (ai-fallback): ${latest.path} (${latest.size} bytes)`); } catch {}
              const resp: any = { success: true, message: note, hex: latest.path, size: latest.size, durationMs: Date.now() - plainStarted, aiDisabled: true };
              sendJson(res, 200, resp);
              sseBroadcast('ai-build-succeeded', resp);
              return;
            }
            try { logger.info(`[AI-BUILD] HEX ready (ai-fallback): ${defaultHexPath} (${stats.size} bytes)`); } catch {}
            const resp: any = { success: true, message: note, hex: defaultHexPath, size: stats.size, mtime: stats.mtime, durationMs: Date.now() - plainStarted, aiDisabled: true };
            sendJson(res, 200, resp);
            sseBroadcast('ai-build-succeeded', resp);
          });
        });
      ...
      let openaiRaw: any = aiResp1.raw; openaiRawAny = openaiRaw;
      ...
      if (strict && aiCode && containsDisallowedForStrict(aiCode)) {
        ...
        aiCode = aiResp2.code;
        if (debug) openaiRaw = aiResp2.raw;
        openaiRawAny = openaiRaw;
      }
      if (debug) {
        logger.debug('[DEBUG][AI] Prompt used:\n' + prompt);
        if (openaiRaw) {
          try { logger.debug('[DEBUG][OpenAI] Response:' + JSON.stringify(openaiRaw).slice(0, 4000)); } catch { logger.debug('[DEBUG][OpenAI] Response (non-JSON)'); }
        }
        sseBroadcast('ai-debug', { phase: 'prompt', prompt });
        if (mcpDebug) sseBroadcast('ai-debug', { phase: 'mcp', stdout: (mcpDebug.stdout || '').slice(0, 2000), stderr: (mcpDebug.stderr || '').slice(0, 2000) });
        if (openaiRaw) sseBroadcast('ai-debug', { phase: 'openai', status: aiResp1.status, snippet: JSON.stringify(openaiRaw).slice(0, 1000) });
        ...
        const logTxt = [
          ...
          `\n--- OpenAI raw (truncated) ---\n${(() => { try { return JSON.stringify(openaiRawAny).slice(0, 8000); } catch { return String(openaiRawAny).slice(0, 8000); } })()}`,
          `\n--- Generated Code ---\n${aiCode}`
        ].join('\n');
        const f = writeDebugLogSync(logsDir, 'ai-build', logTxt);
  if (f) logger.debug('[DEBUG][AI] Wrote debug log: ' + f);
      }
    }
    ...
    runBuild((err2: any, stdout: string, stderr: string) => {
        if (err2) {
      try { if (err2 && err2.stack) logger.error('[BUILD][ai-empty][stack]\n' + String(err2.stack)); } catch {}
      const resp: any = { success: false, message: 'Build failed (AI empty code)', error: String(err2), stack: (err2 && err2.stack) ? String(err2.stack) : undefined, stderr, stdout, durationMs: Date.now() - plainStarted };
          sendJson(res, 500, resp);
          sseBroadcast('ai-build-failed', resp);
          return;
        }
        fs.stat(defaultHexPath, (statErr: any, stats: any) => {
          if (statErr) {
            const latest = findLatestHex();
            if (!latest) {
              const resp: any = { success: false, message: 'Build completed but HEX not found (AI empty code)', expected: defaultHexPath, builtDir, builtListing: listBuiltDir(), stderr, stdout, durationMs: Date.now() - plainStarted };
              sendJson(res, 500, resp);
              sseBroadcast('ai-build-failed', resp);
              return;
            }
    ...
    if (overwriteMain && fs.existsSync(originalMainPath)) {
      ...
      try { if (!fs.existsSync(backup)) fs.copyFileSync(originalMainPath, backup); } catch { }
    }
  ...
  try {
    ...
    const files: string[] = Array.isArray(pj.files) ? pj.files : [];
    const cleaned = files.filter((n: string) => n !== 'ai.generated.ts' && n !== 'main.ts');
  ...
  ensureTestFilesTo(outFile);
    ...
    runBuild(async (err: any, stdout: string, stderr: string) => {
      if (err) {
        ...
        return runBuild((err2: any, stdout2: string, stderr2: string) => {
                if (err2) {
          try { if (err2 && err2.stack) logger.error('[AI-BUILD][autofix][stack]\n' + String(err2.stack)); } catch {}
          const resp: any = { success: false, message: 'AI build failed (after auto-fix attempt)', error: String(err2), stack: (err2 && err2.stack) ? String(err2.stack) : undefined, stderr: stderr2, stdout: stdout2, outFile, outPath, aiCode: finalCode, autoFixTried: true, durationMs: Date.now() - started };
                  if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
                  sendJson(res, 500, resp);
                  sseBroadcast('ai-build-failed', resp);
                } else {
                  fs.stat(defaultHexPath, (statErr: any, stats: any) => {
                    if (statErr) {
                      const latest = findLatestHex();
                      if (!latest) {
                        const resp: any = { success: false, message: 'AI build completed but HEX not found (after auto-fix)', expected: defaultHexPath, builtDir, builtListing: listBuiltDir(), stderr: stderr2, stdout: stdout2, outFile, outPath, aiCode: finalCode, autoFixTried: true, durationMs: Date.now() - started };
                        if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
                        sendJson(res, 500, resp);
                        return;
                      }
                      const norm = normalizeHexOutputs() || { path: latest.path, size: latest.size };
                      try { logger.info(`[AI-BUILD] HEX ready (after auto-fix): ${norm.path} (${norm.size} bytes)`); } catch {}
                      const resp: any = { success: true, message: 'AI build succeeded (after auto-fix)', hex: norm.path, size: norm.size, outFile, outPath, aiCode: finalCode, autoFixTried: true, durationMs: Date.now() - started };
                      if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
                      sendJson(res, 200, resp);
                      sseBroadcast('ai-build-succeeded', resp);
                      return;
                    ...
                    const norm = normalizeHexOutputs() || { path: defaultHexPath, size: stats.size };
                    try { logger.info(`[AI-BUILD] HEX ready (after auto-fix): ${norm.path} (${norm.size} bytes)`); } catch {}
                    const resp: any = { success: true, message: 'AI build succeeded (after auto-fix)', hex: norm.path, size: norm.size, mtime: stats.mtime, outFile, outPath, aiCode: finalCode, autoFixTried: true, durationMs: Date.now() - started };
                    if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
                    sendJson(res, 200, resp);
                    sseBroadcast('ai-build-succeeded', resp);
                  });
  ...
  try { if (err && err.stack) logger.error('[AI-BUILD][stack]\n' + String(err.stack)); } catch {}
  const resp: any = { success: false, message: 'AI build failed', error: String(err), stack: (err && err.stack) ? String(err.stack) : undefined, stderr, stdout, outFile, outPath, aiCode: formattedAiCode, durationMs: Date.now() - started };
        if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
        sendJson(res, 500, resp);
        sseBroadcast('ai-build-failed', resp);
        return;
      ...
      fs.stat(defaultHexPath, (statErr: any, stats: any) => {
        if (statErr) {
          const latest = findLatestHex();
          if (!latest) {
            const resp: any = { success: false, message: 'AI build completed but HEX not found', expected: defaultHexPath, builtDir, builtListing: listBuiltDir(), stderr, stdout, outFile, outPath, aiCode: formattedAiCode, durationMs: Date.now() - started };
            if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
            sendJson(res, 500, resp);
            return;
          }
          const norm = normalizeHexOutputs() || { path: latest.path, size: latest.size };
          try { logger.info(`[AI-BUILD] HEX ready: ${norm.path} (${norm.size} bytes)`); } catch {}
          const resp: any = { success: true, message: 'AI build succeeded', hex: norm.path, size: norm.size, outFile, outPath, aiCode: formattedAiCode, durationMs: Date.now() - started };
          if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
          sendJson(res, 200, resp);
          sseBroadcast('ai-build-succeeded', resp);
          return;
        ...
        const norm = normalizeHexOutputs() || { path: defaultHexPath, size: stats.size };
        try { logger.info(`[AI-BUILD] HEX ready: ${norm.path} (${norm.size} bytes)`); } catch {}
        const resp: any = { success: true, message: 'AI build succeeded', hex: norm.path, size: norm.size, mtime: stats.mtime, outFile, outPath, aiCode: formattedAiCode, durationMs: Date.now() - started };
        if (debug) resp.debug = { prompt: userPrompt, strict, mcp: mcpDebug };
        sendJson(res, 200, resp);
        sseBroadcast('ai-build-succeeded', resp);
      });
    ...
    try { if (e && e.stack) logger.error('[AI-BUILD][top][stack]\n' + String(e.stack)); } catch {}
...
function writeJsonFile(p: string, obj: any) {
  fs.writeFileSync(p, JSON.stringify(obj, null, 2), 'utf8');
}

function ensureFileInPxt(rel: string) {
  const pjPath = path.join(projectDir, 'pxt.json');
  const pj = readJsonFile(pjPath) || {};
  const files: string[] = pj.files || [];
...
function ensureTestFilesTo(rel: string) {
  try {
    const pjPath = path.join(projectDir, 'pxt.json');
    const pj = readJsonFile(pjPath) || {};
    const tf: string[] = Array.isArray(pj.testFiles) ? pj.testFiles : [];
    // If already the only test file, skip writes.
    if (tf.length === 1 && tf[0] === rel) return;
    pj.testFiles = [rel];
    writeJsonFile(pjPath, pj);
    try { logger.info(`[PXT] testFiles set to [${rel}]`); } catch {}
  } catch {}
}

function mcpQueryExamples(mcpDir: string, repoRoot: string, query: string, debug?: boolean): Promise<{ sections?: string[], snippets?: string, raw?: string, _debug?: any }> {
  ...
  const timeout = setTimeout(() => {
      ...
      proc.stdout.once('data', () => {
        ...
        try { firstResp = JSON.parse(out.split('\n').filter((l: string) => l.trim()).shift() || '{}'); } catch { }
        if (debug) {
          logger.debug('[DEBUG][MCP] stdout: ' + (out?.slice(0, 4000) || ''));
          if (err) logger.warn('[DEBUG][MCP] stderr: ' + (err?.slice(0, 2000) || ''));
        }
...
function buildAIPrompt(examples: any, userPrompt: string, strict?: boolean, extraNote?: string): string {
...
function shouldStrict(prompt: string): boolean {
...
function containsDisallowedForStrict(code: string): boolean {
...
function tryRuleBasedGenerate(prompt: string): string | null {
...
function callOpenAIWithRaw(prompt: string, model: string, temperature: number, debug?: boolean): Promise<{ code: string, raw: any, status: number }> {
  return new Promise((resolve, reject) => {
    ...
    const req = requester.request(opts, (resp: any) => {
      let buf = '';
      resp.on('data', (d: any) => { buf += d.toString(); });
      resp.on('end', () => {
        try {
          const j = JSON.parse(buf);
          const content = (((j || {}).choices || [])[0] || {}).message?.content || '';
          const cleaned = stripCodeFences(String(content || ''));
          if (debug) { try { logger.debug('[DEBUG][OpenAI] Status: ' + resp.statusCode + ' Body: ' + buf.slice(0, 4000)); } catch { } }
...
function normalizeZoombitHeadlight(src: string, prompt: string): string {
  if (!src) return src;
  ...
  src = src.replace(/zoombit\.setHeadlight\s*\(([^)]*)\)/g, (m, args) => {
    const a = String(args || '');
    if (/HeadlightChannel\./.test(a) && /digitalStatePicker\s*\(/.test(a)) return m; // already correct
...
async function maybeFormatTs(src: string): Promise<string> {
  try {
    const prettier = require('prettier');
...
function selectEditableFile(): string | null {
  const pjPath = path.join(projectDir, 'pxt.json');
  const pj = readJsonFile(pjPath) || {};
  const files: string[] = pj.files || [];
  if (files.includes('ai.generated.ts')) return 'ai.generated.ts';
  if (files.includes('main.ts')) return 'main.ts';
  const ts = (files || []).find((n: string) => /\.ts$/i.test(n));
  return ts || null;
}

function callOpenAIForFix(args: { code: string, filename: string, errors: string, model: string }): Promise<string> {
